...
public class VarTest {
    //使用static修饰的是类变量，所有实例共享
    //count用于记录创建该类的实例个数
    public static int count = 0;

    //没有使用stati修饰的是实例变量，每个实例有单独的拷贝，index用于存放当前是累的第一个实例
    public int index = 0;
    ...
    public void println() {
        String longStr = String.format("当前创建第%d个实例，共%d个", index, count);
